...
export default async function fetchCharity(name) {
  const now = Date.now()
  const res = await fetch(new URL(`causes/search/Canada/${name}`, BENEVITY_BASE))
